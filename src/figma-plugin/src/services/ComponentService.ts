...
export class ComponentService {
  ...
  async getLocalComponents(): Promise<ComponentInfo[]> {
    try {
      ...
      // Load all pages first to access components across all pages
      ...
      const componentInfos = components.map(component => {
      ...
      console.log('✅ Local components processed:', {
        totalComponents: componentInfos.length,
        components: componentInfos.map(comp => ({
          name: comp.name,
          id: comp.id,
      ...
      console.error('❌ Error getting local components:', error);
  ...
  async createComponentInstance(params: CreateComponentInstanceParams): Promise<NodeInfo> {
    ...
    try {
      ...
      // Load all pages first to access components across all pages
      await figma.loadAllPagesAsync();
      console.log('📄 All pages loaded for component search');
      ...
      if (!component) {
        console.error('❌ Component not found with key:', componentKey);
  ...
  async setInstanceOverrides(params: SetInstanceOverridesParams): Promise<setInstanceOverridesResult> {
    ...
    const sourceMainComponent = await sourceInstance.getMainComponentAsync();
    ...
    for (const targetNodeId of targetNodeIds) {
      try {
        ...
        const targetMainComponent = await targetInstance.getMainComponentAsync();
        if (targetMainComponent && targetMainComponent.id !== sourceMainComponent.id) {
